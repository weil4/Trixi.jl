...
function create_cache(mesh::TriangularMesh, equations,
    ...
    triangulation_neighbors = delaunay_compute_neighbors(data_points, triangulation_vertices)

    volume, dx = calc_volume(data_points, triangulation_vertices, mesh, equations, solver)

    midpoints = calc_midpoints(data_points, triangulation_vertices, mesh, equations, solver)

    cache = (; data_points, triangulation_vertices, triangulation_neighbors, volume, dx, midpoints)

    return cache
end

function calc_volume(data_points, triangulation_vertices, mesh::TriangularMesh, equations, solver::FV)
    (; n_elements) = mesh
    ...
    for element in axes(triangulation_vertices, 2)
        ...
        dx[element] = max(norm(x_node1 - x_node2), norm(x_node1 - x_node3), norm(x_node2 - x_node3))
    end
...
function calc_midpoints(data_points, triangulation_vertices, mesh, equations, solver)
    midpoints = Matrix{eltype(data_points)}(undef, ndims(mesh), size(triangulation_vertices, 2))
    for element in axes(triangulation_vertices, 2)
        node1 = triangulation_vertices[1, element]
        node2 = triangulation_vertices[2, element]
        node3 = triangulation_vertices[3, element]
...
function create_cache(mesh::VoronoiMesh2, equations,
                      solver::FV, RealT, uEltype)
    (; data_points) = mesh
    # n_dims = ndims(mesh)
    ...
    voronoi_vertices_coordinates, voronoi_vertices, voronoi_vertices_interval = build_polygon_mesh(data_points, triangulation_vertices, mesh_type=mesh.mesh_type,
                                                                                         orthogonal_boundary_edges=mesh.orthogonal_boundary_edges)
    mesh.n_voronoi_elements = size(voronoi_vertices_interval, 2)

    voronoi_neighbors = voronoi_compute_neighbors(triangulation_vertices, voronoi_vertices, voronoi_vertices_interval, triangulation_neighbors)

    volume, dx = calc_volume(voronoi_vertices_coordinates, voronoi_vertices, voronoi_vertices_interval, mesh, equations, solver)

    cache = (; data_points, voronoi_vertices_coordinates, voronoi_vertices, voronoi_vertices_interval, voronoi_neighbors, volume, dx)

    return cache
end

function calc_volume(voronoi_vertices_coordinates, voronoi_vertices, voronoi_vertices_interval, mesh::VoronoiMesh2, equations, solver::FV)
    ...
    for element in eachindex(volume)
        ...
        for i in node_index_start:(node_index_end-1)
            ...
            volume[element] += voronoi_vertices_coordinates[1, node1] * voronoi_vertices_coordinates[2, node2] -
                               voronoi_vertices_coordinates[2, node1] * voronoi_vertices_coordinates[1, node2]
        end
        node_last = voronoi_vertices[node_index_end]
        node_first = voronoi_vertices[node_index_start]
        volume[element] += voronoi_vertices_coordinates[1, node_last] * voronoi_vertices_coordinates[2, node_first] -
                           voronoi_vertices_coordinates[2, node_last] * voronoi_vertices_coordinates[1, node_first]
        volume[element] *= 0.5
...
function allocate_coefficients(mesh::VoronoiMesh2, equations, solver::FV, cache)
    # We must allocate a `Vector` in order to be able to `resize!` it (AMR).
    # cf. wrap_array
    zeros(eltype(cache.data_points),
          nvariables(equations) * nelements(mesh, solver, cache))
...
function compute_coefficients!(u, func, t, mesh::VoronoiMesh2, equations,
                               solver::FV, cache)
    (; data_points) = cache
    for element in eachelement(mesh, solver, cache)
...
function rhs!(du, u, t, mesh::VoronoiMesh2, equations, initial_condition,
              boundary_conditions, source_terms::Source, solver::FV,
              cache) where {Source}
